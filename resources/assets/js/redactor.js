...
(function($)
{
	...
	$.fn.redactor = function(options)
	{
		...
		else
		{
			this.each(function()
			{
				if (!$.data(this, 'redactor')) $.data(this, 'redactor', new Redactor(this, options));
			});
		}
	...
	Redactor.fn = $.Redactor.prototype = {
		...
		buildAfter: function()
		{
			...
			// focus
			if (this.opts.focus) setTimeout($.proxy(this._focus, this), 100);
		...
		buildBindKeyboard: function()
		{
			this.$editor.on('keydown.redactor', $.proxy(function(e)
			{
				...
				// shortcuts setup
				if (ctrl && !e.shiftKey && !e.altKey) this.shortcuts(e, key);
				...
				{
					if (this.o.rebuffer.length != 0) this.bufferRedo();
				...
				if (key == this.keyCode.ENTER && !e.shiftKey && !e.ctrlKey && !e.metaKey )
				{
					...
					if (parent.nodeType == 1 && (this.browser('msie') || this.browser('opera'))
			...
			this.$editor.on('keyup.redactor', $.proxy(function(e)
			{
				...
				{
					if (key == this.keyCode.A) this.selectall = true;
					else if (key != this.keyCode.LEFT_WIN && key != this.keyCode.CTRL) this.selectall = false;
				}

				// replace to p before / after the table or body
				if (this.opts.paragraphy && current.nodeType == 3 && (parent == false || parent.tagName == 'BODY'))
				{
		...
		toolbarBuild: function()
		{
			...
			if (this.opts.toolbar)
			{
				if (this.opts.linebreaks)
				{
					...
					if (num === 0) this.opts.formattingTags.splice(num, 1);
				}
		...
		dropdownBuild: function($dropdown, dropdownObject)
		{
			$.each(dropdownObject, $.proxy(function(btnName, btnObject)
			{
				...
				else
				{
					...
					{
						if (btnObject.callback) btnObject.callback.call(this, e, btnObject, btnName);
		...
		buttonBuild: function(btnName, btnObject)
		{
			...
			$button.on('click', $.proxy(function(e)
			{
				...
				{
					this[btnObject.func](e);
					this.airBindMousemoveHide();

				}
				else if (btnObject.callback)
				{
					btnObject.callback.call(this, e, btnObject, btnName);
					this.airBindMousemoveHide();
				...
				if ($list.length)
				{
					...
					$.each(elems, $.proxy(function(i,s)
					{
						if (s.tagName == 'LI')
						{
							...
							else data += this.outerHtml(cloned.contents()) + '<br>';
		...
		cleanReConvertProtected: function(html)
		{
			...
			html = html.replace(/<section(.*?) rel="redactor-form-tag">([\w\W]*?)<\/section>/gi, '<form$1>$2</form>');
		...
		cleanUnverified: function(saveFocus)
		{
			if (saveFocus) this.selectionSave();

			this.$editor.find('span[data-redactor!="verified"]').filter(':not([id*="selection-marker"])').contents().unwrap();

			if (saveFocus) this.selectionRestore(false);

			var $elem = this.$editor.find('img, a, b, strong, i, em, u, strike, span')
		...
		formatBlocks: function(tag)
		{
			...
			$.each(nodes, $.proxy(function(i, node)
			{
				if (node.tagName !== 'LI')
				{
					this.formatBlock(tag, node, false);
				}
		...
		formatBlock: function(tag, block, offset)
		{
			...
			if (offset !== false) var offset = this.getCaretOffset(block);

			var contents = '';
			if (tag !== 'pre') contents = $(block).contents();
			else contents = this.cleanEncodeEntities($(block).text());
			...
			else
			{
				...
				if (offset !== false) this.setCaret(node[0], offset);
			}
		},
		formatChangeTag: function(fromElement, toTagName, save)
		{
			...
			if (save !== false) this.selectionRestore(false);
		...
		insertHtml: function (html, sync)
		{
			...
			if (this.opts.paragraphy && $html.contents().length == 1 && $html.contents()[0].nodeType == 3
			...
			if (this.selectall)
			{
				...
				{
					if (this.opts.paragraphy) this.selectionEnd(this.$editor.contents().last());
					else this.focusEnd();
		...
		pasteInsert: function(html)
		{
			if (this.selectall)
			{
				if (this.opts.paragraphy) this.$editor.html(this.opts.emptyHtml);
		...
		getCaretOffset: function (element, trim)
		{
			var caretOffset = 0;
		...
		selectionRestore: function(replace, remove)
		{
			if (!this.opts.rangy)
			{
				if (!this.savedSel) return false;

				if (replace !== false && this.savedSel) this.$editor.html(this.savedSel);
		...
		tableInsert: function()
		{
			...
			this.tableObserver(this.$editor.find('#table' + tableId));
			this.buttonActiveObserver();
		},
		...
		linkShow: function()
		{
			...
			var callback = $.proxy(function()
			{
				...
				if (sel && sel.anchorNode && par.tagName === 'A')
				{
					url = par.href;
					text = par.text;
					target = par.target;

					if (sel.toString() === '') this.insert_link_node = par;
		...
		linkProcess: function()
		{
			...
			if (tab_selected === '1')
			{
				...
				if ($('#redactor_link_blank').prop('checked')) target = ' target="_blank"';
			...
			this.linkInsert('<a href="' + link + '"' + target + '>' + text + '</a>', $.trim(text), link, target);
		...
		modalInit: function(title, content, width, callback)
		{
			...
			$redactorModal.find('header').html(title);
		...
		{
			if (this.opts.s3) this.handleFileSelect(e);
			else this.uploadForm(this.element, this.uploadFrame());
		},
		...
		{
			this.uploadProgress( 0, 'Upload started.');

			var files = e.target.files || e.dataTransfer.files;

			for (var i = 0, f; f = files[i]; i++)
			{
				if (this.opts.s3) this.s3_uploadFile(f);
				else this.draguploadUpload(f);

			}
		},
		s3_createCORSRequest: function(method, url)
		{
		...
		s3_uploadFile: function (file)
		{
			$.getJSON(this.opts.s3 + '?name=' + file.name + '&type=' + file.type ).done($.proxy(function(json)
			{
				json.url = decodeURIComponent(json.url);
				this.s3_uploadToS3(file, json);

			}, this))
			.fail($.proxy(function(jqXHR, textStatus)
			{
				this.uploadProgress(0, 'Could not contact signing script. Status = ' + textStatus);

			}, this));
		},
		s3_uploadToS3: function(file, json)
		{
			var xhr = this.s3_createCORSRequest('PUT', json.url);

			if (!xhr)
			{
				this.uploadProgress( 0, 'CORS not supported');
			}
			else
			{
				xhr.onload = $.proxy(function ()
				{
					if (xhr.status == 200)
					{
						this.uploadProgress(100, 'Upload completed.');

						this.uploadOptions.success({ filelink: json.file });

					}
					else
					{
						this.uploadProgress(0, 'Upload error: ' + xhr.status);
					}
				}, this);

				xhr.onerror = $.proxy(function()
				{
					this.uploadProgress(0, 'XHR error.');
				...
				xhr.upload.onprogress = $.proxy(function(e)
				{
					...
					{
						var percentLoaded = parseInt(e.loaded / e.total * 100, 10);
						this.uploadProgress(e, percentLoaded == 100 ? 'Finalizing.' : 'Uploading.');
					}
				}, this );
